...
export const load: PageServerLoad = async ({ locals }) => {
    const [plans, latestStrategy] = await Promise.all([
        listWeekPlans(locals.supabase, 10),
        listStrategyReports(locals.supabase, 1),
    ])
...
export const actions: Actions = {
    generate: async ({ locals }) => {
        ...
        const [profile, resumes, strategies, backlog] = await Promise.all([
            ...
            listStrategyReports(locals.supabase, 1),
            listByPriorities(locals.supabase, ['backlog']),
        ])
        ...
        try {
            ...
            await saveWeekPlan(locals.supabase, {
                model: plan.model,
                body: plan.body,
    ...
    delete: async ({ request, locals }) => {
        ...
        await deleteWeekPlan(locals.supabase, id)
