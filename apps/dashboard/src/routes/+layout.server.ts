...
export const load: LayoutServerLoad = async ({ locals }) => {
    ...
    if (!user) {
        return { accountLabel: null, loggedIn: false }
    }
    ...
    if (isTelegramEmail(user.email) && typeof handle === 'string' && handle.length > 0) {
        return { accountLabel: `@${handle}`, loggedIn: true }
    }
    return { accountLabel: isTelegramEmail(user.email) ? 'Telegram' : (user.email ?? null), loggedIn: true }
}
